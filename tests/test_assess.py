...
POLICY = ruleset("expense_policy", within_limit=rule("No employee may claim over $5,000 in total."))
...
def test_playbooks_keep_declaration_order_and_reject_empties():
    pb = ruleset("p", a=rule("x"), b=rule("y"), c=rule("z"))
...
def test_hints_are_offered_as_non_exhaustive():
    c = plausibility("shipment", notable=["delivery date vs dispatch date"])
...
def test_custom_levels_are_respected():
    c = plausibility("record", levels=["fine", "odd", "wrong", "impossible"])
    assert c["levels"] == ["fine", "odd", "wrong", "impossible"]


def test_plausibility_check_pairs_a_priority_score_with_a_location():
    ...
    assert options == list(DEFAULT_PLAUSIBILITY_ASPECTS)
...
def test_aspects_are_generic_not_edge_cases():
    ...
    assert set(DEFAULT_PLAUSIBILITY_ASPECTS) == {
        "nothing", "time", "amounts", "activity", "status", "identity"
    }


def test_plausibility_check_accepts_a_domain_taxonomy():
    pb = plausibility_check("invoice", aspects={"nothing": "fine", "tax": "VAT looks wrong"})
    assert [o["name"] for o in pb.rules["where"]["options"]] == ["nothing", "tax"]


def test_a_plausibility_ruleset_runs_through_assess(mock_server):
    def responder(body):
        answers = {}
    ...
    out = assess(df, plausibility_check("employee record"), by="id")
...
def test_one_row_and_one_request_per_group(mock_server, ledger):
    out = assess(ledger, POLICY, by="employee")
    assert out.height == 3
...
def test_each_group_is_sent_as_an_array_of_records(mock_server, ledger):
    assess(ledger, POLICY, by="employee", records=["date", "amount_usd"])
...
def test_context_constants_reach_every_group(mock_server, ledger):
    assess(ledger, POLICY, by="employee", context={"limit_usd": 5000, "period": "2026-03"})
...
def test_every_rule_rides_in_the_same_request(mock_server, ledger):
    ...
    pb = ruleset("p", a=rule("x"), b=rule("y"), c=rule("z"))
    out = assess(ledger, pb, by="employee")
...
def test_grouping_by_several_keys(mock_server, ledger):
    out = assess(ledger, POLICY, by=["employee", "category"])
...
def test_usage_columns_are_opt_in(mock_server, ledger):
    out = assess(ledger, POLICY, by="employee", usage=True)
...
def test_the_warning_threshold_is_tunable_and_quiet_when_respected(mock_server):
    ...
    with _w.catch_warnings():
        ...
        assess(big, POLICY, by="g", max_rows_per_group=100)
...
def test_misuse_is_rejected_before_any_request(mock_server, ledger):
    with pytest.raises(TypeError, match="must be a RuleSet"):
        assess(ledger, {"a": rule("x")}, by="employee")
    with pytest.raises(ValueError, match="at least one row"):
        assess(ledger.head(0), POLICY, by="employee")
    with pytest.raises(ValueError, match="not a column"):
        assess(ledger, POLICY, by="nope")
    with pytest.raises(ValueError, match="records columns not in the frame"):
        assess(ledger, POLICY, by="employee", records=["nope"])
    with pytest.raises(ValueError, match="collides with a group key"):
        assess(ledger, POLICY, by="employee", compute={"employee": pl.len()})
