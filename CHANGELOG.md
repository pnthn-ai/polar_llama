...
- **Self-consistency checks** (`consistency`, `self_consistency`, `DEFAULT_REVIEW_LEVELS`, `DEFAULT_CONSISTENCY_ASPECTS`): the playbook shape for rules you *cannot write down in advance* -- "does it make sense that someone has worked here three years and never taken leave?". Nobody writes that rule, and writing it only creates the need for the next one (the intern on the top pay band, the rep with record commission and no customer meetings, the employee on leave who closed a full year of tickets); the anomaly that matters is always the one nobody predicted. So no rules are written: `self_consistency(subject)` asks whether a record hangs together across all its dimensions and returns a triage ranking. Three design decisions are measured, not assumed. (1) **A score, not a yes/no.** On four planted inconsistencies of four different kinds against four clean controls, the noul framing ("is this record coherent?") caught 2 of 4 -- it hedges toward "plausible" because most fields in an odd record genuinely are fine -- while the graded "how strongly does this warrant review?" caught 4 of 4 with zero false positives (clean 0.07/0.09/0.13/0.18 vs planted 0.98/1.44/1.72/1.82). `consistency()` therefore returns a score. (2) **Record width dominates everything else.** The same four people with the same planted issues ranked *inverted* at 5 fields (clean 0.83/1.10 vs planted 0.45/1.23) and correctly at 12 fields (clean 0.14/0.27 vs planted 1.08/0.57) -- consistency is a relationship between fields, so few fields means few relationships to be wrong. (3) **Column names are part of the prompt**: renaming `pto_days` (taken? or remaining?) to `pto_days_TAKEN_last_12_months`, changing nothing else, roughly doubled the separation. `self_consistency` pairs the priority score with a `where` choice over a small GENERIC aspect taxonomy (time / amounts / activity / status / identity) -- naming aspects tells a reviewer where to look without constraining what counts as a problem, and was correct 7/8 with the one "miss" defensible. `where_confidence` ran 0.63-0.81 on clean controls and 0.25-0.29 on the subtler planted cases, so low confidence marks what most deserves a person's eyes rather than what to discard. Includes a written operational playbook, `docs/CONSISTENCY_PLAYBOOK.md`, covering how to run it, how to read it, what it is not (a ranking for attention, never a verdict), and how to validate it on your own data before trusting it.
