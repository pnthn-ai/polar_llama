...
### Added
- **Rule assessment across many rows** (`assess`, `ruleset`, `rule`, `RuleSet`): business rules evaluated against *many rows at once*, the third fan-out shape alongside `typesafe_eval` (one row, N questions) and `typesafe_eval_each` (one contract per segment). A ruleset judges a **group of rows together**, which is the only shape that can answer a rule no single row can violate -- "no employee may claim more than $5,000 in total", "every escalated ticket must get a support reply". Pure Python over existing primitives (`polar_llama/assess.py`): rows are grouped, each group's rows become a `List[Struct]` that `typesafe_eval`'s existing structured-state handling sends as a JSON array of records, and every rule in the ruleset is answered in the SAME request per group -- so N rules cost no extra requests. Groups go out in parallel under the existing `POLAR_LLAMA_MAX_CONCURRENCY` bound, inheriting the TypeSafe client's 429/529/5xx backoff-with-jitter retry and per-group `_error` isolation. **Zero new Rust.** `rule(statement, passes=..., fails=...)` frames a noul as an adherence question (the answer is the probability the rows *adhere*, so violations threshold at `< 0.5`); `ruleset(name, **rules)` is a named, reusable rule set; `assess(df, ruleset, by=..., records=..., compute=..., context=..., usage=..., max_rows_per_group=...)` returns one row per group with the group keys, `row_count`, anything computed, one Float64 column per rule, and `_error`. Two API decisions are driven by measurement against the live API rather than assumption: (1) **signal dilutes with group size** -- on a planted violation, clean vs violating separated at 6 rows (0.33/0.98), was marginal at 20 (0.40/0.49) and *indistinguishable* by 60 (0.20/0.39), 150 (0.18/0.20) and 300 (0.18/0.22), and this is NOT specific to arithmetic (a purely semantic rule, one un-answered ticket among many, held at 11 rows and was missed at 59), so `max_rows_per_group` (default 25) warns rather than silently returning noise; (2) **Polars should do the arithmetic** -- judging a pre-computed aggregate instead of raw rows to add up was exact at every size tested (0.03 vs 0.98 from 20 through 1,000 rows summarised) because the state stays small however many rows it covers, which is what `compute=` exists for. The state a group receives is always a named object (`records`, `row_count`, plus anything from `compute`/`context`); `row_count` is always sent partly so the shape cannot depend on whether `compute` happened to be passed, and both names are reserved. DataFrame-level orchestration like `quality_report` / `induce_codebook`, deliberately NOT added to the `.llama` expression namespace. See `docs/ASSESSMENT.md`.
- **Plausibility checks** (`plausibility`, `plausibility_check`, `DEFAULT_REVIEW_LEVELS`, `DEFAULT_PLAUSIBILITY_ASPECTS`): the ruleset shape for rules you *cannot write down in advance* -- "does it make sense that someone has worked here three years and never taken leave?". Nobody writes that rule, and writing it only creates the need for the next one (the intern on the top pay band, the rep with record commission and no customer meetings, the employee on leave who closed a full year of tickets); the anomaly that matters is always the one nobody predicted. So no rules are written: `plausibility_check(subject)` asks whether a record hangs together across all its dimensions and returns a triage ranking. Three design decisions are measured, not assumed. (1) **A score, not a yes/no.** On four planted inconsistencies of four different kinds against four clean controls, the noul framing ("is this record coherent?") caught 2 of 4 -- it hedges toward "plausible" because most fields in an odd record genuinely are fine -- while the graded "how strongly does this warrant review?" caught 4 of 4 with zero false positives (clean 0.07/0.09/0.13/0.18 vs planted 0.98/1.44/1.72/1.82). `plausibility()` therefore returns a score. (2) **Record width dominates everything else.** The same four people with the same planted issues ranked *inverted* at 5 fields (clean 0.83/1.10 vs planted 0.45/1.23) and correctly at 12 fields (clean 0.14/0.27 vs planted 1.08/0.57) -- consistency is a relationship between fields, so few fields means few relationships to be wrong. (3) **Column names are part of the prompt**: renaming `pto_days` (taken? or remaining?) to `pto_days_TAKEN_last_12_months`, changing nothing else, roughly doubled the separation. `plausibility_check` pairs the priority score with a `where` choice over a small GENERIC aspect taxonomy (time / amounts / activity / status / identity) -- naming aspects tells a reviewer where to look without constraining what counts as a problem, and was correct 7/8 with the one "miss" defensible. `where_confidence` ran 0.63-0.81 on clean controls and 0.25-0.29 on the subtler planted cases, so low confidence marks what most deserves a person's eyes rather than what to discard. Includes a written operational playbook, `docs/PLAUSIBILITY_PLAYBOOK.md`, covering how to run it, how to read it, what it is not (a ranking for attention, never a verdict), and how to validate it on your own data before trusting it.
